...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const sclrnProizv = x1 * x2 + y1 * y2;
  const mod1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const mod2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cosA = sclrnProizv / (mod1 * mod2);
...
function roundToPowerOfTen(num, pow) {
  const base = num / 10 ** pow;
  const b = base.toFixed(0);
  return b + '0'.repeat(pow);
}
 ...
 */
function isPrime(n) {
  const arrayFromZeroToN = Array.from(
    { length: n - 2 },
    (_, index) => index + 1 + 1
  );
  if (arrayFromZeroToN.filter((el) => n % el === 0).length > 0) {
    return false;
  }
  return true;
}
...
function getSumToN(n) {
  const arrayFromOnetoN = Array.from({ length: n }, (_, ind) => ind + 1);
  return arrayFromOnetoN.reduce((sum, curr) => sum + curr, 0);
}
...
function isPowerOfTwo(num) {
  const arrOfTwoStep = Array.from(
    { length: num - 1 },
    (_, index) => 2 ** index
  );
  return arrOfTwoStep.includes(num);
}
...
function getSumOfNumbers(x1, x2, x3) {
  return [x1, x2, x3].reduce((cur, sum) => sum + cur, 0).toFixed(3);
}
